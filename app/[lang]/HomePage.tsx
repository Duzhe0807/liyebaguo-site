...
export function HomePage({ locale }: { locale: Locale }) {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    fetch("/api/submit", {
      ...
      .catch(() => {
        ...
        setNotice(locale === "zh" ? "提交未成功，请稍后重试，或通过电话、邮件与微信联系。" : "Submission failed. Please retry or contact us by phone, email or WeChat.");
      });
  };
  ...
  return (
    <main>
      ...
      <div className="mobile-cta" aria-label={locale === "zh" ? "快捷联系" : "Quick contact"}><a href="#booking" onClick={() => setBookingMode("guest")}>{locale === "zh" ? "立即预约" : "Book"}</a><a href="#booking" onClick={() => setBookingMode("group")}>{locale === "zh" ? "团队咨询" : "Group"}</a><a href="#contact">WhatsApp</a></div>
    </main>
  );
}
