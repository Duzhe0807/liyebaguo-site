    ...
    <main className="home-page">
      <header className="site-header">
        ...
        <div className="header-actions">
          ...
          <Link className="button compact" href={`${langPath(locale)}/checkout`}>{t.book}</Link>
      ...
      <section className="hero">
        ...
        <div className="hero-copy">
          ...
          <div className="button-row">
            <Link className="button" href={`${langPath(locale)}/checkout`}>{t.book}</Link>
      ...
      <section className="section conversion" id="booking">
        ...
        <div className={bookingMode === "guest" ? "mobile-form-active online-ticket-entry" : "online-ticket-entry"} id="guest-form"><p className="eyebrow">OFFICIAL ONLINE TICKETS</p><h2>{t.booking}</h2><p>{locale === "zh" ? "在线选择日期、午宴或晚宴场次及席位，系统会实时确认线上余票并锁定 15 分钟。" : "Choose a date, lunch or dinner session and seats online. Available inventory is confirmed and held for 15 minutes."}</p><Link className="button" href={`${langPath(locale)}/checkout`}>{locale === "zh" ? "进入官方购票" : "Book official tickets"}<ArrowRight /></Link></div>
      ...
      <div className="mobile-cta" aria-label={locale === "zh" ? "快捷联系" : "Quick contact"}><Link href={`${langPath(locale)}/checkout`} className="primary">{t.book}</Link><CustomerServiceChooser compact isEnglish={locale === "en"} /><a href="tel:+8617383017612">{locale === "zh" ? "电话" : "Call"}</a></div>
    </main>
  );
}
